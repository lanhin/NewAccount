...
import com.lanhin.account.adapter.BillSwipeAdapter;
...
public class BillDetailActivity extends BaseActivity {
    ...
    protected void initData() {
        ...
        if (time != 0l) {
            mDate = new Date(time);
            mAccountList.addAll(getAccountList(0, mDate, Extra.ACCOUNT_TYPE_EXPEND));
        }
    }
        ...
        mRgType.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                switch (checkedId) {
                    case R.id.rb_expend:
                        if (mDate != null)
                            changeList(Extra.ACCOUNT_TYPE_EXPEND);
                        break;
                    case R.id.rb_income:
                        if (mDate != null)
                            changeList(Extra.ACCOUNT_TYPE_INCOME);
                        break;
                    default:
                        break;
                }
